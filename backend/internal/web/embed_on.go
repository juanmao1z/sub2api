...
const (
	...
	defaultHTMLCacheTTL  = 30 * time.Second
)
...
func (s *FrontendServer) serveIndexHTML(c *gin.Context) {
	...
	// Check cache first. A TTL covers settings changed outside SetOnUpdateCallback.
	cached := s.cache.Get()
	if cached != nil && s.isCacheFresh(cached) {
...
func (s *FrontendServer) isCacheFresh(cached *CachedHTML) bool {
	if cached == nil {
		return false
	}
	if s.cacheTTL <= 0 || cached.CachedAt.IsZero() {
		return true
	}
	return time.Since(cached.CachedAt) < s.cacheTTL
}
