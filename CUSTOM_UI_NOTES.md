...
- 当前目标镜像：`sub2api-custom:0.1.155-ui3`
- 当前定制基线：官方 `v0.1.155` + 本仓库自定义 `/recharge` 页面、`usage-leaderboard` 自定义菜单入口与首页实时并发状态
...
2. 基于官方版本创建新的定制分支，例如 `codex/update-v0.1.155-custom`。
3. 只重放必要的定制改动：
   - `/recharge` 前端页面
   - 侧边栏入口
   - i18n 文案
...
6. 打包到 `deploy-artifacts/`，例如 `deploy-artifacts/sub2api-custom-0.1.155-ui3-context.tar.gz`。
...
若新镜像上线后异常，把 `/opt/sub2api-deploy/docker-compose.override.yml` 的镜像改回上一个可用标签 `sub2api-custom:0.1.155-ui2`，或恢复本次部署前的 `bak-v0155-ui3` 备份，然后执行：
