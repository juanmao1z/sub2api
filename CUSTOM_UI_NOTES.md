...
- 当前目标镜像：`sub2api-custom:0.1.155-ui2`
...
6. 打包到 `deploy-artifacts/`，例如 `deploy-artifacts/sub2api-custom-0.1.155-ui2-context.tar.gz`。
...
若新镜像上线后异常，把 `/opt/sub2api-deploy/docker-compose.override.yml` 的镜像改回上一个可用标签 `sub2api-custom:0.1.153-ui2`，或恢复本次部署前的 `bak-v0155` 备份，然后执行：
