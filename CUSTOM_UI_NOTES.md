...
- 当前升级目标镜像：`sub2api-custom:0.1.156-ui2`
- 当前回滚镜像：`sub2api-custom:0.1.156-ui1`
- 当前定制基线：官方 `v0.1.156` + 本仓库自定义品牌首页、默认 `/home` 登录跳转、社群弹窗、`/recharge` 页面、`usage-leaderboard` 自定义菜单入口与首页实时并发状态
...
6. 本地用 `Dockerfile.prebuilt-binary` 构建 `sub2api-custom:0.1.156-ui2`；Docker Hub 不可用时，用 `Dockerfile.rebase-binary` 基于 `0.1.156-ui1` 离线重建。
7. 本地执行 `docker save`、压缩并计算 SHA-256，上传到 `/opt/sub2api-images/0.1.156-ui2`。
...
若新镜像上线后异常，把 `/opt/sub2api-deploy/docker-compose.override.yml` 的镜像改回上一个可用标签 `sub2api-custom:0.1.156-ui1`，或恢复本次部署前的 `bak-v0156-ui2` 备份，然后执行：
