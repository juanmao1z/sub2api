...
`src/views/HomeView.vue` 及其依赖保持原实现。构建首先运行原有 Vite 流程，然后由 `assemble.mjs` 合并已固定版本的 Kedaya 前端资源。`/home` 使用原入口，其他路由使用 Kedaya 入口；跨入口导航会重新加载文档，以隔离两套全局样式。账号会话仍使用现有 localStorage 字段。
...
构建产物位于 `backend/internal/web/dist`。`integration/build-manifest.json` 记录首页原始哈希与适配文件哈希。
