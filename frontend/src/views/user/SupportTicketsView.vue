...
const { t } = useI18n(); const appStore = useAppStore(); const tickets = ref<SupportTicket[]>([]); const loading = ref(false); const submitting = ref(false); const showCreate = ref(false); const selected = ref<SupportTicketDetail | null>(null); const reply = ref(''); const form = reactive<{type:'REFUND'|'SUGGESTION';subject:string;description:string;order_id?:number}>({ type:'SUGGESTION', subject:'', description:'' })
...
async function createTicket() { submitting.value = true; try { await supportTicketsAPI.create({ ...form, ...(form.type === 'REFUND' && form.order_id ? { order_id: form.order_id } : {}) }); showCreate.value = false; form.subject=''; form.description=''; form.order_id=undefined; await load() } catch (e) { appStore.showError(extractI18nErrorMessage(e, t, 'support.errors', t('common.error'))) } finally { submitting.value = false } }
