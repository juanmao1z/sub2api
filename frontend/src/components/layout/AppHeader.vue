<template>
  <header class="signal-header sticky top-0 z-30 border-b border-gray-200 bg-white dark:border-dark-700 dark:bg-dark-900">
    <div class="header-inner flex h-16 items-center justify-between gap-2 px-2 sm:px-4 md:px-6">
      ...
      <div class="header-location flex shrink-0 items-center gap-2 sm:gap-4">
        ...
        <div v-if="useConsoleTheme" class="signal-breadcrumb">
          <router-link :to="authStore.isAdmin ? '/admin/dashboard' : '/dashboard'" class="signal-breadcrumb-home">{{ appStore.siteName }}</router-link>
          <Icon name="chevronRight" size="sm" />
          <span>{{ pageTitle }}</span>
        </div>
        <div v-else class="hidden lg:block">
          <h1 class="text-lg font-semibold text-gray-900 dark:text-white">{{ pageTitle }}</h1>
          <p v-if="pageDescription" class="text-xs text-gray-500 dark:text-dark-400">{{ pageDescription }}</p>
        </div>
      </div>

      <!-- Right: Announcements + Docs + Language + Subscriptions + Balance + User Dropdown -->
      <div class="header-actions flex min-w-0 items-center gap-1 sm:gap-3">
        <ConsoleSearch v-if="useConsoleTheme" />
        <!-- Announcement Bell -->
...
import ConsoleSearch from '@/components/layout/ConsoleSearch.vue'
